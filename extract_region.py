...
bigsnapshot = ".../snap_028_z000p000.0.hdf5"
output = 'galaxy1'

#gas,halo,0,0,stars,blackhole
#types = 1,1,0,0,1,1
types = None #gets them all

datasets = None #gets them all
#datasets = ('Coordinates','Velocity','ParticleIDs')

#define cube size (cMpc/h)
load_region_length = 2

#Extract conversion factors
#-------------------------
f = h5py.File(bigsnapshot, 'r')
CGS  = []
AEXP = []
HEXP = []
ITYPE = []
ATT = []
attributes = ['Coordinates','Velocity','Mass']
for itype in range(5):
    for i, att in enumerate(attributes):
        print ("Particle type: %i, attribute: %s"%(itype, att))
        try:
            ...
            print("%s not found."%attributes[i])
            CGS.append(np.nan)
            AEXP.append(np.nan)
            HEXP.append(np.nan)
            ITYPE.append(itype)
            ATT.append(att)
np.savetxt('conversion_factors_%s'%output, transpose([ITYPE, CGS, AEXP, HEXP]), fmt='%.2e', delimiter=',', header='Particle Type, CGS, AEXP, HEXP\nFor each particle type: Coordinates, Velocity, Mass')
#--------------------------

h = f['Header'].attrs.get('HubbleParam')

#get galaxy centre from database and insert here
xCOP = 13.560003
yCOP = 20.400774
zCOP = 15.675034

centre = np.array ([xCOP*h,yCOP*h,zCOP*h])
...
if __name__ == "__main__":
    ...
    extract_region(fname, sample_rate, region, datasets, output, types)
